...
int strCmpCnt(char* str1, char* str2) {
    ...
    // Count differing characters up to shorter length
    for (i = 0; i < minlen; i++) {
        if (str1[i] != str2[i]) {
            diff++;
        }
    ...
    // Add leftover characters from the longest string
    diff += abs(len1 - len2);
...
int strCmpInd(char* str1, char* str2) {
    ...
    for (int i = 0; i < minlen; i++) {
        if (str1[i] != str2[i]) {
            return i; // first difference found
        }
    }

    // if same up to the shorter string but lengths differ
    if (len1 != len2) {
        return minlen;
    }
...
bool checkForValidWord(char** words, int numWords, int wordSize, WordNode* ladder, char* aWord) {
    ...
    // 4. Must differ by exactly one character
    WordNode* top = ladder;
    if (top != NULL) {
        int diff = strCmpCnt(aWord, top->myWord);
        if (diff != 1) {
...
WordNode* copyLadder(WordNode* ladder) {
    ...
    WordNode* currOld = ladder->next;
    WordNode* currNew = newHead;

    while (currOld != NULL) {
        WordNode* newNode = (WordNode*)malloc(sizeof(WordNode));
        newNode->myWord = currOld->myWord;
        newNode->next = NULL;
...
//-------------------------------------------------------------------
    ...
    // Print top there "..." first
    printf("  ...\n  ...\n  ...\n");

    // From head to tail (latest->earlist)
    WordNode* curr = ladder;
    while(curr != NULL) {
        printf("  %s\n", curr->myWord);
        curr = curr->next;
...
//-------------------------------------------------------------------
    ...
    // Store all nodes into an array for reverse printing
    WordNode* curr = ladder;
    while (curr != NULL) {
        // Print the current word
        printf("  %s\n", curr->myWord);

        // If there is a next word, print an arrow and add a blank line
        if (curr->next != NULL) {
            int diffIndex = strCmpInd(curr->myWord, curr->next->myWord);
            int wordLen = strlen(curr->myWord);

            // Print indentation, align arrows with differnet characters
            printf("  ");
            for (int j = 0; j < diffIndex; j++) {
                printf(" ");
            }
            printf("^");
            // Print the remaining spaces to align the length of the line with the word line
            for (int j = diffIndex + 1; j < wordLen; j++) {
                printf(" ");
            }
...
WordNode* popLadderFromFront(LadderNode** list) {
    ...
    LadderNode* temp = *list;  // first node
    WordNode* poppedLadder = temp->topWord;

    *list = temp->next;  // move head to next node
    free(temp);  // free the popped LadderNode (not the ladder it first)

    return poppedLadder;  // return the WordNode* (ladder)
}
...
void freeLadderList(LadderNode* myList) {
    ...
    while (curr != NULL) {
        ...
        if (temp->topWord != NULL) {
            freeLadder(temp->topWord);   // free the ladder inside
        }
        free(temp);                      // free the LadderNode itself
    }
}
...
WordNode* findShortestWordLadder(   char** words, 
    ...
    // Create the first ladder with just startWord
    ...
    // BFS loop
    while (queue != NULL) {
        // Get the front ladder
        WordNode* currLadder = popLadderFromFront(&queue);
        char* topWord = currLadder->myWord;

        // Explore all dictionary words
        for (int i = 0; i < numWords; i++) {
            if (!usedWord[i] && strCmpCnt(topWord, words[i]) == 1) {
                ...
                if (strcmp(words[i], finalWord) == 0) {
                    ...
                    freeLadderList(queue);  // clear queue
                    return newLadder;
                }

                // Otherwise, enqueue new ladder
                insertLadderAtBack(&queue, newLadder);
            }
        }

        // Done expanding this ladder
        freeLadder(currLadder);
    }
